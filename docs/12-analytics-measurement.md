...
| `tool_used` | `components/tools/geo-scan-form.tsx`, `components/tools/diagnoo-form.tsx` | `slug`, `locale` — tarama başlatıldı (yanıt beklenmeden) |
| `tool_scan_completed` | `components/tools/geo-scan-form.tsx`, `components/tools/diagnoo-snapshot.tsx` | `slug`, `band`, `locale` — tarama skorla tamamlandı |
| `tool_report_requested` | `components/tools/geo-report-form.tsx`, `components/tools/diagnoo-unlock-form.tsx` | `slug`, `band`, `locale` — detaylı rapor isteği (e-posta + KVKK rızası) başarıyla gönderildi |
