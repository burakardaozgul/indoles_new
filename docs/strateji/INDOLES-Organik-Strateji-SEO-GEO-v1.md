# INDOLES Organik Büyüme Stratejisi — SEO + GEO v1.15

> **Statü:** Onaylı (v1.14 — 2026-09-03, Diagnoo lansmanı — Faz 2 Görev 10). v1.15 uygulama kaydıdır; içindeki tek açık karar (hizmet `seo.title`ına "CRO ajansı" girsin mi) Burak onayı bekliyor.
...
| v1.14 | 2026-09-03 | **Diagnoo canlıya alındı — sırlar gelmeden, bilinçli olarak (Faz 2 Görev 10).** Burak kararı: "gizli olmasın, indeksimizi alalım, kullanıma sonra açarız zaten şu an trafik yok." `DIAGNOO_TOOL.published`, üç sır (`GEMINI_API_KEY`, `FIRECRAWL_API_KEY`, `PSI_API_KEY`) ve uzak D1 migration'ının (0004-0007) `--remote` koşusu BEKLENMEDEN `false` → `true` yapıldı — trafik sıfırken indeksleme saatini şimdi işletmenin maliyeti yok, kullanım sırlar gelince açılır. Somut sonuç: `/tr/araclar/diagnoo` + `/en/tools/diagnoo` sitemap'te (URL sayısı 144 → 146), `/araclar` indeksinde iki kart, `llms.txt`'te listeli, sayfa artık `noindex, nofollow` döndürmüyor (`tool` profiline giriyor — FAQPage + SoftwareApplication JSON-LD, ≥300 kelime, ≥4 iç link). Üçgenin her iki ayağı da canlı: hizmet→araç (`toolsForService`, `cro`/`performans-pazarlama`/`e-ticaret` sayfalarında callout) ve araç→makale (`bridgesForArticle`, altı makale + GAP analizi rehberi, yedi köprü). Sırlar ve migration gelene kadar canlı araç her taramaya dürüst `scrape_failed` hatasıyla cevap verir (production'da doğrulandı, Faz 1) — "yakında" ekranı yok, kapsam dışı bir durum değil. `/araclar` indeksinin `META`'sı iki araçlı kataloğu anlatacak şekilde yeniden yazıldı (sayı vermeden, `indoles-brand-voice` ile). Kalan owner adımları (üç sır + migration + gerçek anahtarla tek koşu) `docs/runbooks/diagnoo-ga4-kurulum.md`'de güncel. |
| v1.13 | 2026-09-03 | **Diagnoo araç niyeti kelimeleri yerleşti (Faz 2 Görev 5).** Araç sayfası ARAÇ niyetini hedefler — `e-ticaret site analizi`, `CRO analizi`, `dönüşüm oranı analizi`, `ücretsiz e-ticaret denetimi`; bilgi niyeti (`dönüşüm optimizasyonu`, `cro ajansı`, `e ticaret danışmanlığı`) CRO ve e-ticaret hizmet/rehber sayfalarında KALIR — aynı kanibalizasyon disiplini (A-6). İlk üçü `keyword-coverage.test.ts`'e araç çifti olarak eklendi (`TARGETS_TOOLS`); dördüncüsü niyet sınıflandırması olarak kayda geçti, sayfa kendi `teşhis` terminolojisini koruduğu için birebir cümle olarak zorlanmadı. Üçgenin hizmet→araç ayağı (`toolsForService`) ve araç→makale köprüleri (`bridgesForArticle`) artık kod (Faz 2 Görev 1-2), yayın kapısına bağlı — `DIAGNOO_TOOL.published` `false` sürdükçe hiçbir arama yüzeyinde görünmez. GEO ölçüm rutini değişmedi: `GEO-Olcum-Rutini.md` §6-§7 disiplini sürüyor, araç sayfaları yeni bir prompt serisi açmadan K3/S3 promptlarının "Atıf verilen URL" kolonunda izlenir. |
| v1.12 | 2026-09-02 | Araç ④ UI v2 — tek sayfa üç durum, skor kartı ve ölçek, kilitli düzeltme listesi, derleme zamanı OG skor kartları (ADR-031), araç rotalarında popup yok; hedef kelimeler değişmedi (`TARGETS_TOOLS` yeşil). |
...
| v1.10 | 2026-08-30 | **Off-site otorite planı + araç stratejisi onaylandı (Burak).** Backlink yaklaşımı "kazanılan atıf" olarak tanımlandı — link satın alma/PBN/dizin spamı reddedildi; GEO baz çizgisinin (Ay 0: 0/30) ana teşhisi off-site sinyal yokluğu. Dizin-varlık katmanı ve 6 araçlık portföy `Off-Site-Otorite-ve-Arac-Plani.md`'ye çıkarıldı; development kapasitesi SEO/GEO varlık üretimine bağlandı (Burak: "siteye istediğimiz kadar kullanıcı çekecek tool ekleyebiliriz"). §5'e plan referansı, §8 Faz 2 satırına onay işlendi — `/araclar` yeniden açılıyor, **ADR + docs/02 güncellemesi dev tarafında bekliyor**. Hedef: 90 günde +15-20 kaliteli referring domain. Ayrıca GBP + Bing/IndexNow kuruldu (30 Ağu, Burak teyidi). |
...
| **Araç sayfaları — GEO** | `geo-gorunurluk-denetleyicisi` canlı; araç niyeti kelimeleri (`GEO denetimi`, `AI görünürlük testi`, `llms.txt kontrolü`) `keyword-coverage.test.ts`'te korunur | Düşük taahhütlü ziyaretçiyi tek oturumda teşhis + CTA'ya çevirir (A-6) |
| **Araç sayfaları — Diagnoo** | `diagnoo` canlıda (`published: true`, 2026-09-03, Faz 2 Görev 10) — sırlar ve migration gelmeden, bilinçli olarak; araç niyeti kelimeleri (`e-ticaret site analizi`, `cro analizi`, `dönüşüm oranı analizi`) aynı testte korunur | Hizmet→araç→makale üçgeni (Faz 2 Görev 1-2) artık canlı: aynı teşhis + CTA mantığı CRO/e-ticaret/performans pazarlama için çalışıyor; gerçek veri sırlar gelene kadar `scrape_failed` ile beklemede |
...
> **v1.10 güncellemesi:** Bu tablonun "varlık tutarlılığı" satırı somutlaştı ve genişledi — dizin/varlık başvuru listesi, kazanılan-atıf ilkeleri ve interaktif araç portföyü `Off-Site-Otorite-ve-Arac-Plani.md`'de. Bing/IndexNow ve GBP kuruldu (30 Ağu).
...
| Faz 2 → **Onaylandı (v1.10)** | İnteraktif araçlar (`/araclar`) — Burak onayıyla yeniden açıldı; 6 araçlık portföy ve yapım sırası `Off-Site-Otorite-ve-Arac-Plani.md` §2'de. **Uygulama önkoşulu: ADR + `docs/02` route güncellemesi** | Araçlar hem lead hem kazanılan-link üretir — bütçesiz dönemin en güçlü otorite varlığı; development kapasitesi buraya bağlandı |
