...
function shell(body: string, locale: (typeof LOCALES)[number]): string {
  ...
  // çalışır, "DENETLEYICISI"/"İYI" gibi bozuk büyük harfler basılır — aynı
  // hata kök layout'ta da vardı (bkz. src/app/layout.tsx:95).
...
async function main(): Promise<void> {
  ...
  async function shoot(
    ...
    await page.setContent(shell(renderToStaticMarkup(element), locale), { waitUntil: "networkidle" });
  ...
  for (const locale of LOCALES) {
    for (let score = 0; score <= 100; score += 1) {
      await shoot(path.join(OUT, locale, `${score}.png`), createElement(GeoCard, { score, locale }), locale);
    }
    await shoot(path.join(OUT, locale, "tool.png"), createElement(ToolCard, { locale }), locale);
