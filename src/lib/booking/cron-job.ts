...
export async function runDailyCronJob(env: CronEnv): Promise<CronResult> {
  ...
  const deletedCount = await deleteBookingsOlderThan(db, cutoffUtc);
  ...
  const completedCount = await completePastBookings(db, nowUtc);
  ...
  const orphans = await listOrphanedBookings(db, nowUtc);
  if (orphans.length > 0) {
    try {
      ...
      reportError(err, { route: "cron", step: "orphan-report" });
    }
  }
  ...
  //    tutmayı gerektirmiyor ve maliyeti tek bir küçük istek.
  try {
