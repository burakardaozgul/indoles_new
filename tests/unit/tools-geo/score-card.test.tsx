...
describe("ScoreCard", () => {
  ...
  it("skoru, bandı ve bant cümlesini basar; reduced-motion'da sayaç anında biter", () => {
    render(<ScoreCard result={RESULT} tool={GEO_TOOL} locale="tr" shareUrl="https://x/y" />);
    ...
    expect(screen.getByText(GEO_TOOL.bands["gelismeye-acik"].tr)).toBeInTheDocument();
  ...
  it("onNewScan verilirse düğme, newScanHref verilirse link basar", () => {
    ...
    const { rerender } = render(
      <ScoreCard result={RESULT} tool={GEO_TOOL} locale="tr" shareUrl="u" onNewScan={onNewScan} />,
    );
    fireEvent.click(screen.getByRole("button", { name: "Yeni tarama" }));
    expect(onNewScan).toHaveBeenCalledTimes(1);
    rerender(<ScoreCard result={RESULT} tool={GEO_TOOL} locale="tr" shareUrl="u" newScanHref="/tr/araclar/geo" />);
  ...
  it("sayaç animasyonluyken de son değere ulaşır", async () => {
    ...
    render(<ScoreCard result={RESULT} tool={GEO_TOOL} locale="tr" shareUrl="u" />);
    await waitFor(
      () => expect(screen.getByText("55", { selector: "[data-part='score']" })).toBeInTheDocument(),
      { timeout: 2000 },
    );
